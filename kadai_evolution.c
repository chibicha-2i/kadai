...
void enqueue(QUEUE1 *queue, int input){
    ...
    if(l==(queue->back+1)%MAX_NUMqueue){
        printf("キューが満杯でENQUEUEできません\n");
        return;
    }
   
    queue->data[(queue->back+1)%MAX_NUMqueue]=input;
    queue->back=(queue->back+1)%MAX_NUMqueue;
}

int dequeue(QUEUE1 *queue){
    int ret=0;
    
    if((queue->back + 1) % MAX_NUMqueue == queue->head){
        printf("キューが空です\n");
        printf("%d\n",queue->head);
        return -1;
    }
    ...
    queue->head = (queue->head + 1) % MAX_NUMqueue;
    return ret;
